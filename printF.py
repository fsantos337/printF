...
def fechar():
    ...
    for exe_path, processo in list(processos_ativos.items()):
        try:
            if processo.poll() is None:  # Se ainda está rodando
                print(f"Terminando processo: {exe_path}")
                
                # Tenta terminar de forma amigável primeiro
                processo.terminate()
                
                # Aguarda um pouco para o processo fechar
                for i in range(5):  # Tenta por 5 segundos
                    ...
                    # Se não fechar, força o término
                    processo.kill()
                    processos_terminados += 1
                    print(f"Processo {exe_path} forçado a fechar")
                    
        except Exception as e:
            print(f"Erro ao fechar processo {exe_path}: {e}")
    
    # Limpa o dicionário
    processos_ativos.clear()
    
    print(f"Processos terminados: {processos_terminados}")
    
    cleanup_lock()
    root.destroy()
    
    # Força saída completa do programa
    os._exit(0)
